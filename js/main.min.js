...
function ScalesItem(a){this.id=a;this.scale="major";this.root="C";this.semiTones=DEFAULT_SCALE_SEMITONES;this.scaleNotes=DEFAULT_SCALE_NOTES;this.tuning="standart_e";this.halfStep=0;this.stringsNumber=3;this.stringsTunes=DEFAULT_STRING_TUNES;this.isTriadMode=!1;this.putNotesOnString=function(a){var b=this.getTuneForString(a),c=$("."+HOR_FRET_CLASS+":eq("+a+")",$("."+NULL_VER_FRET_CLASS+", ."+VER_FRET_CLASS,this.$fretboardBlock),this.$fretboardBlock);a=getSemiTonesPatternForString(this.scaleNotes,
this.semiTones,b);c=$("."+NOTE_CLASS,c);c.css("display","none");c.toggleClass(TRANSPARENT_NOTE_CLASS,!1);for(var e=a.shift(),b=nextNoteAfterSemiTones(b,e),f=0;e<c.length;){var g=$(c[e]);g.css("display","inline");if(this.isTriadMode){var h=this.scaleNotes.indexOf(b)+1;1!=h&&3!=h&&5!=h&&g.toggleClass(TRANSPARENT_NOTE_CLASS,!0);g.text(h)}else g.text(b);b==this.root?g.toggleClass(NORMAL_NOTE_CLASS,!1).toggleClass(HIGHLIGHTED_NOTE_CLASS,!0):g.toggleClass(NORMAL_NOTE_CLASS,!0).toggleClass(HIGHLIGHTED_NOTE_CLASS,
!1);e+=a[f];b=nextNoteAfterSemiTones(b,a[f]);f=++f%a.length}};this.putNotesOnAllStrings=function(){for(var a=0;a<this.stringsNumber;a++)this.putNotesOnString(a)};this.incStringsNumber=function(){this.stringsNumber++;this.$stringsNumberBlock.text(""+this.stringsNumber)};this.decStringsNumber=function(){this.stringsNumber--;this.$stringsNumberBlock.text(""+this.stringsNumber)};this.getTuneForString=function(a){return this.stringsTunes[a%this.stringsTunes.length]};this.addString=function(a){var b=this.getTuneForString(a),
c=$("."+STRINGS_OPTIONS_BLOCK_CLASS,this.$itemBlock).append(STRING_TUNE_BLOCK_TMPL()).children().last();$("."+STRING_TUNE_SELECT_CLASS+" :contains('"+b+"')",c).prop("selected",!0);for(var b={currentStringNumber:a+1},e=$("."+NULL_VER_FRET_CLASS+", ."+VER_FRET_CLASS,this.$fretboardBlock),f,g,h=0;h<e.length;h++)$(e[h]).append(STRING_FRET_TMPL(b)),0==a&&(g=h%12,f=3==g||5==g||7==g||10==g,g=0!=h&&0==g,f?$(e[h]).append(STRING_FRET_MARK_TMPL()):g&&$(e[h]).append(STRING_DOUBLE_FRET_MARK_TMPL()));this.putNotesOnString(a);
$("."+STRING_TUNE_SELECT_CLASS,c).change({itemThis:this},this.onStringTuneChange);$("."+LEFT_ARROW_CLASS,c).click({itemThis:this},this.onLeftArrowTuneClick);$("."+RIGHT_ARROW_CLASS,c).click({itemThis:this},this.onRightArrowTuneClick)};this.delLastString=function(){$("."+STRING_TUNE_BLOCK_CLASS,this.$itemBlock).last().remove();for(var a=$("."+NULL_VER_FRET_CLASS+", ."+VER_FRET_CLASS,this.$fretboardBlock),d=0;d<a.length;d++)$("."+HOR_FRET_CLASS,$(a[d])).last().remove()};this.selectCurrentStringsTunes=
function(){for(var a=$("."+STRINGS_OPTIONS_BLOCK_CLASS,this.$itemBlock),d=0;d<this.stringsNumber;d++)a.find("."+STRING_TUNE_SELECT_CLASS+":eq("+d+") :contains('"+this.getTuneForString(d)+"')").prop("selected",!0)};this.selectCurrentTuning=function(){this.$itemBlock.find("."+TUNING_SELECT_CLASS+" [value='"+this.tuning+"']").prop("selected",!0)};this.selectCurrentScale=function(){$("."+SCALE_SELECT_CLASS+" [value='"+this.scale+"']",this.$itemBlock).prop("selected",!0)};this.selectCurrentHalfStep=function(){$("."+
HALF_STEP_SELECT_CLASS+" [value='"+this.halfStep+"']",this.$itemBlock).prop("selected",!0)};this.moveTuning=function(a){for(var b=0;b<this.stringsTunes.length;b++)this.stringsTunes[b]=nextNoteAfterSemiTones(this.stringsTunes[b],a)};this.changeScaleNotesBlock=function(){$("."+SCALE_NOTES_CLASS,this.$itemBlock).remove();var a=this.scaleNotes;a.push(this.root);a={root:this.root,semiTones:this.semiTones,notes:a};$("."+SCALE_NOTES_BLOCK_CLASS,this.$itemBlock).append(SCALE_NOTES_TMPL(a))};this.onScaleChange=
...
this.onAddStringButton=function(a){a=a.data.itemThis;var b=a.stringsNumber;b<MAX_STRINGS_NUMBER&&(a.addString(b),a.incStringsNumber())};this.onDelStringButton=function(a){a=a.data.itemThis;a.stringsNumber>MIN_STRINGS_NUMBER&&(a.delLastString(),a.decStringsNumber())};this.onCloseButton=function(a){a=a.data.itemThis;a.$itemBlock.hide(200,function(){$(this).remove()});deleteItem(a.id)};this.onSetDefaultButton=function(a){a=a.data.itemThis;defaultScaleItemOptions.scale=a.scale;defaultScaleItemOptions.root=
a.root;defaultScaleItemOptions.semiTones=a.semiTones;defaultScaleItemOptions.tuning=a.tuning;defaultScaleItemOptions.halfStep=a.halfStep;defaultScaleItemOptions.stringsTunes=a.stringsTunes;defaultScaleItemOptions.stringsNumber=a.stringsNumber;defaultScaleItemOptions.isTriadMode=a.isTriadMode;Cookies.set("defaultScaleOptions",defaultScaleItemOptions,{expires:DEFAULT_SCALE_OPTIONS_EXPIRE_DAYS})};this.onTriadsCheckboxClick=function(a){a=a.data.itemThis;a.isTriadMode?(a.isTriadMode=!1,a.$triadsCheckbox.hide(0),
a.$triadsCheckboxEmpty.show(0)):(a.isTriadMode=!0,a.$triadsCheckboxEmpty.hide(0),a.$triadsCheckbox.show(0));a.putNotesOnAllStrings()};this.initStrings=function(){for(var a=0;a<FRETS_NUMBER;a++)this.$fretboardBlock.children().last().before(STRING_VER_FRET_TMPL());for(a=0;a<this.stringsNumber;a++)this.addString(a),this.$stringsNumberBlock.text(""+this.stringsNumber);this.selectCurrentStringsTunes()};this.readDefaultScaleItemOptions=function(){this.scale=defaultScaleItemOptions.scale;this.root=defaultScaleItemOptions.root;
this.semiTones=getScaleSemitones(this.scale);this.scaleNotes=getNotesFromSemiTones(this.root,this.semiTones);this.tuning=defaultScaleItemOptions.tuning;this.halfStep=defaultScaleItemOptions.halfStep;this.stringsTunes=defaultScaleItemOptions.stringsTunes;var a=defaultScaleItemOptions.stringsNumber;a<MIN_STRINGS_NUMBER&&(a=MIN_STRINGS_NUMBER);this.stringsNumber=a;this.isTriadMode=defaultScaleItemOptions.isTriadMode};this.initTriadsCheckbox=function(){this.$triadsCheckbox=$("."+TRIADS_CHECKBOX_CLASS,
this.$itemBlock);this.$triadsCheckboxEmpty=$("."+TRIADS_CHECKBOX_EMPTY_CLASS,this.$itemBlock);this.isTriadMode?this.$triadsCheckboxEmpty.hide(0):this.$triadsCheckbox.hide(0);this.$triadsCheckbox.click({itemThis:this},this.onTriadsCheckboxClick);this.$triadsCheckboxEmpty.click({itemThis:this},this.onTriadsCheckboxClick)};this.initAnimation=function(){this.$itemBlock.hide(0);this.$itemBlock.show(200)};this.init=function(){$addNewItemBtn.before(SCALES_ITEM_BLOCK_TMPL({id:this.id}));this.$itemBlock=$("#"+
this.id);this.$stringsNumberBlock=$("."+STRING_NUMBER_CLASS,this.$itemBlock);this.$fretboardBlock=$("."+FRETBOARD_CLASS,this.$itemBlock);this.initAnimation();this.readDefaultScaleItemOptions();this.initStrings();this.initTriadsCheckbox();this.changeScaleNotesBlock();this.selectCurrentTuning();this.selectCurrentScale();this.selectCurrentHalfStep();this.putNotesOnAllStrings();$("."+SCALE_SELECT_CLASS,this.$itemBlock).change({itemThis:this},this.onScaleChange);$("."+TUNING_SELECT_CLASS,this.$itemBlock).change({itemThis:this},
this.onTuningChange);$("."+HALF_STEP_SELECT_CLASS,this.$itemBlock).change({itemThis:this},this.onHalfStepChange);this.$itemBlock.find("."+HALF_STEP_BLOCK_CLASS).find("."+LEFT_ARROW_CLASS).click({itemThis:this},this.onLeftArrowHalfStepClick);this.$itemBlock.find("."+HALF_STEP_BLOCK_CLASS).find("."+RIGHT_ARROW_CLASS).click({itemThis:this},this.onRightArrowHalfStepCLick);$("."+SCALE_NOTES_BLOCK_CLASS,this.$itemBlock).on("click","."+ROOT_NOTE_CLASS,{itemThis:this},this.onRootNoteChange);$("."+ADD_STRING_BTN_CLASS,
this.$itemBlock).click({itemThis:this},this.onAddStringButton);$("."+DEL_STRING_BTN_CLASS,this.$itemBlock).click({itemThis:this},this.onDelStringButton);$("."+CLOSE_BTN_CLASS,this.$itemBlock).click({itemThis:this},this.onCloseButton);$("."+SET_DEFAULT_BTN_CLASS,this.$itemBlock).click({itemThis:this},this.onSetDefaultButton)};this.init()};var menuItems=[],usedIDs=[];function getIDforNewItem(){for(var a=0;0<=usedIDs.indexOf("menuItem"+a);)a++;return"menuItem"+a}function createNewItem(){var a=getIDforNewItem();menuItems.length<MAX_ITEMS_NUMBER?(menuItems.push(new ScalesItem(a)),usedIDs.push(a)):$addNewItemBtn.hide(200)}function deleteItem(a){menuItems.length==MAX_ITEMS_NUMBER&&$addNewItemBtn.show(200);for(var b=0;b<menuItems.length;b++)menuItems[b].id===a&&(menuItems.splice(b,1),usedIDs.splice(usedIDs.indexOf(a),1))}
